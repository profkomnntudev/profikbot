...
const mainMarkup = [
	...
	[Markup.button(phrase.hellomk.inst, "primary"), Markup.button(phrase.hellomk.tonntu[0], "primary")],
	[Markup.button(phrase.hellomk.exchangeintonntu, "primary"), Markup.button(phrase.hellomk.exchangeonnntu, "primary")],
...
bot.command(["Начать", "Привет", "Добрый день", phrase.start.to_start, phrase.start.anotherq, "Start"], async ctx => {
	...
	await ctx.reply(text, img, Markup.keyboard(mainMarkup).oneTime(true));
});
...
bot.command(["спасибо", "Благодарю"], async ctx => {
	await ctx.reply(
		phrase.thx[RandInt(0, phrase.thx.length - 1)],
		null,
		Markup.keyboard([[Markup.button(phrase.start.anotherq, "positive")]]).oneTime(true)
	);
});

bot.command("пока", async ctx => {
	await ctx.reply(
		phrase.bye[RandInt(0, phrase.bye.length - 1)],
		null,
		Markup.keyboard([[Markup.button(phrase.start.anotherq, "positive")]]).oneTime(true)
	);
});
...
bot.command(phrase.money.wait, async ctx => {
	...
	await ctx.reply(text, null, Markup.keyboard([[Markup.button(phrase.start.anotherq, "positive")]]).oneTime(true));
});
...
bot.on(async ctx => {
	...
	let partner =
		msgtxt.indexOf("https://vk.com/wall") != -1 ||
		msgtxt.indexOf("w=wall") != -1 ||
		msgtxt.indexOf("разместить") != -1 ||
		msgtxt.indexOf("опубликовать") != -1 ||
		msgtxt.indexOf("пост") != -1 ||
		msgtxt.indexOf("репост") != -1 ||
		msgtxt.indexOf("конкурс") != -1 ||
		msgtxt.indexOf("разместить") != -1;
	if (partner) {
	...
	if (att || partner) {
		await ctx
			.reply(text, null, Markup.keyboard(mainMarkup).oneTime(true))
			.then(
				partner &&
					bot.sendMessage(
						db.getLeader("iao").id,
						"vk.com/id" + ctx.message.from_id + " Чекни сообщения группы, тут сотрудничество предлагают"
					)
			);
	} else {
		db.deleteAction(ctx.message.from_id);
